...
function tur_account(){
 ...
 $posts=get_posts(array('post_type'=>'post','post_status'=>'publish','numberposts'=>100,'has_password'=>false,'category_name'=>'cr-news'));$preferred=array();$discovery=array();$map=array();$history=$p['remember']?(array)get_user_meta($uid,'_tur_history',true):array();$read=array();
...
add_action('admin_post_tur_preferences',function(){
 ...
 update_user_meta($uid,'_tur_preferences',$p);if(tur_preferences($uid)!==$p)tue_notice('Your preferences could not be saved. Please try again.');update_user_meta($uid,'_tur_preferences_updated',gmdate('c'));if(!$p['remember'])delete_user_meta($uid,'_tur_history');if(!$old['notify']&&$p['notify'])update_user_meta($uid,'_tur_seen',time());wp_safe_redirect(add_query_arg('saved','1',tur_url()));exit;
});
...
add_action('admin_post_tur_reset',function(){check_admin_referer('tur-reset');$uid=get_current_user_id();foreach(array('_tur_preferences','_tur_history','_tur_seen')as $key)delete_user_meta($uid,$key);update_user_meta($uid,'_tur_preferences_updated',gmdate('c'));wp_safe_redirect(tur_url());exit;});
