import {Engagement} from '../editorial/engagement';
export type WordPressConfig={url:string;username:string;password:string};
export type WordPressPost={todayuk_engagement?:Engagement;id:number;link:string;status:string;modified_gmt:string;title?:{raw?:string};content?:{raw?:string}};
...
export class WordPressClient{
 ...
 async verifyEngagement(id:number,expected:Engagement){const post=await this.read(id),actual=post.todayuk_engagement;if(!actual||actual.mode!==expected.mode||JSON.stringify([...(actual.tools??[])].sort())!==JSON.stringify([...expected.tools].sort())||actual.question!==expected.question||JSON.stringify(actual.options)!==JSON.stringify(expected.options))throw Error('WordPress has not confirmed the approved reader engagement. The article has not been published. Check the TodayUK Engagement plugin.');}
