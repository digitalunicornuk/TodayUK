...
export async function POST(request:Request){
 ...
 try{post=b.action==='send'?await wp.createDraft(delivery.draft_id,delivery.headline,delivery.body,delivery.engagement):await (async()=>{if(delivery.engagement)await wp.verifyEngagement(delivery.wordpress_id,delivery.engagement);return wp.publish(delivery.wordpress_id);})();}catch(e){await c.db.rpc('finish_wordpress_delivery',{p_id:delivery.id,p_state:'uncertain',p_post:null,p_url:null,p_remote:null,p_message:'Transfer outcome needs checking in WordPress. Do not resend.'});throw e;}
